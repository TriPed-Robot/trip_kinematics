...
def plot_forward(robot_type):
    forward_reference = read_csv(os.path.join(
        'tests', 'experiments', robot_type, 'reference_solution', 'endeffector_coordinates.csv'))
    forward_calculated = read_csv(os.path.join(
...
def plot_inverse(robot_type, inv_kin_type):
    inverse_reference = read_csv(os.path.join(
        'tests', 'experiments', robot_type, 'reference_solution', 'joint_values.csv'))
    inverse_calculated = read_csv(os.path.join(
        'tests', 'experiments', robot_type, "inverse_kinematics", inv_kin_type, 'joint_values.csv'))
    ...
    plt.title("Inverse kinematics test type " +
              inv_kin_type+" of robot "+robot_type)
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ...
    plot_inverse("triped", "CCDSolver")
    plot_inverse("triped", "NewtonSolver")
