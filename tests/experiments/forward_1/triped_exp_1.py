...
if __name__ == '__main__':
    ...
    filename_inverse_tip = 'tests/experiments/forward_1/input_data/tip.csv'
    ...
    with open(filename_inverse_tip, newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
            tips.append([float(row[0]),float(row[1]),float(row[2])])



    output_rows = []
    #tip = {'rx': 0, 'ry': 0, 'rz': 0}
    tip = {'rx': 0.2, 'ry': 0.2, 'rz': 0.1}
    for i in range(len(input_t1)):
        tip = {'rx': tips[i][0], 'ry': tips[i][1], 'rz': tips[i][2]}
        ...
        gimbal_joint.set_virtual_state([{},tip])
